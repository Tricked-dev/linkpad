...
export const colors = {
  primary: Object.fromEntries(
    shades.map((x) => [x, `rgba(var(--color-primary-${x}), %alpha)`])
  ),
  secondary: Object.fromEntries(
    shades.map((x) => [x, `rgba(var(--color-secondary-${x}), %alpha)`])
  ),
  tertiary: Object.fromEntries(
    shades.map((x) => [x, `rgba(var(--color-tertiary-${x}), %alpha)`])
  ),
  success: Object.fromEntries(
    shades.map((x) => [x, `rgba(var(--color-success-${x}), %alpha)`])
  ),
  warning: Object.fromEntries(
    shades.map((x) => [x, `rgba(var(--color-warning-${x}), %alpha)`])
  ),
  error: Object.fromEntries(
    shades.map((x) => [x, `rgba(var(--color-error-${x}), %alpha)`])
  ),
  surface: Object.fromEntries(
    shades.map((x) => [x, `rgba(var(--color-surface-${x}), %alpha)`])
  ),
